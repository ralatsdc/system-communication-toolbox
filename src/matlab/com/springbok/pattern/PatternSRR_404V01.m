...
% 
classdef PatternSRR_404V01 < SpacePattern & ReceivePattern
% Describes the ITU antenna pattern SRR_404V01.
  ...
  methods
    ...
    function this = PatternSRR_404V01(Phi0)
    ...
    % Parameters
    %   None
      that = PatternSRR_404V01(this.Phi0);

    end % copy()

    function [G, Gx, fH] = gain(this, Phi, varargin)
    % Appendix 30A (RR-2001) reference receiving space station
    ...
    % Check number and class of input arguments.
      ...
      G1 = this.GainMax - 17.5 - 25 * log10(phi_over_phi0);
      
      G = G0 .* (0   <= phi_over_phi0 & phi_over_phi0 < 1.3) ...
          + G1 .* (1.3 <=  phi_over_phi0);
      
      Gx0 = this.GainMax - 35;
      phix = max(eps, phi_over_phi0 - 1);
      Gx1 = this.GainMax - 40 - 40 * log10(phix);
      
      Gx = Gx0 .* (0    <= phi_over_phi0 & phi_over_phi0 < 1.75) ...
           + Gx1 .* (1.75 <= phi_over_phi0);
      ...
      if this.GainMax < 35 && absolute_pattern
        warning( ...
            'Springbok:InvalidResult', ...
            'GainMax is less than 35.');
