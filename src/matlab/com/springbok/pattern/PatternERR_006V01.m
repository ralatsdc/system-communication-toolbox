...
% 
classdef PatternERR_006V01 < EarthPattern & ReceivePattern
% Describes the ITU antenna pattern ERR_006V01.
  ...
  methods
    ...
    function this = PatternERR_006V01(GainMax, Phi0)
    % Constructs a PatternERR_006V01 given an antenna maximum gain, and cross-sectional half-power beamwidth.
    %
    % Parameters
    ...
    end % PatternERR_006V01()
    
    function that = copy(this)
    % Copies a PatternERR_006V01 given a maximum antenna gain.
    %
    % Parameters
    %   None
      that = PatternERR_006V01(this.GainMax, this.Phi0);

    end % copy()

    function [G, Gx, fH] = gain(this, Phi, varargin)
    % Appendix 30 (RR-2001) reference receiving earth station
    % antenna pattern for Regions 1 and 3 for individual reception
    % (1977 BSS Plan).
    %
    % Parameters:
    ...
    % Check number and class of input arguments.
      ...
      G2 = this.GainMax - 9 - 20 * log10(phi_over_phi0);
      G3 = this.GainMax - 8.5 - 25 * log10(phi_over_phi0);
      G4 = this.GainMax - 33;
      
      G = G0 .* (0     <= phi_over_phi0 & phi_over_phi0 <= 0.25) ...
          + G1 .* (0.25  <  phi_over_phi0 & phi_over_phi0 <= 0.707) ...
          + G2 .* (0.707 <  phi_over_phi0 & phi_over_phi0 <= 1.26) ...
          + G3 .* (1.26  <  phi_over_phi0 & phi_over_phi0 <= 9.55) ...
          + G4 .* (9.55  <  phi_over_phi0);
      
      Gx0 = this.GainMax - 25;
      Gx1 = this.GainMax - 30 - 40 * log10(phix);
      Gx2 = this.GainMax - 20;
      Gx3 = this.GainMax - 30 - 25 * log10(phix);
      Gx4 = this.GainMax - 30;
      
      Gx = Gx0 .* (0    <= phi_over_phi0 & phi_over_phi0 <= 0.25) ...
           + Gx1 .* (0.25 <  phi_over_phi0 & phi_over_phi0 <= 0.44) ...
           + Gx2 .* (0.44 <  phi_over_phi0 & phi_over_phi0 <= 1.4) ...
           + Gx3 .* (1.4  <  phi_over_phi0 & phi_over_phi0 <= 2) ...
           + Gx4 .* (2    <  phi_over_phi0);
