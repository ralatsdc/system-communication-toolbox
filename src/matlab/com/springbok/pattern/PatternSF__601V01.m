classdef PatternSF__601V01 < SpacePattern & TransmitPattern
% Describes the ITU antenna pattern SF__601V01.
...
% Copyright (C) 2016 Springbok LLC <http://springbok.io/> All rights reserved.
  ...
  methods
    ...
    function this = PatternSF__601V01(Phi0)
    % Constructs a PatternSF__601V01 given a maximum antenna gain.
    %
    % Parameters
    ...
    end % PatternSF__601V01()
    
    function that = copy(this)
    % Copies a PatternSF__601V01 given a maximum antenna gain.
    %
    % Parameters
    %   None
      that = PatternSF__601V01(this.Phi0);

    end % copy()

    function [G, Gx, fH] = gain(this, Phi, varargin)
    % Fast roll-off transmitting space station antenna pattern for
    % RADIOSAT-3 submitted by France.
    %
    % Parameters:
    ...
    % Check number and class of input arguments.
      ...
      x = 0.5 * (1 - Beamlet / this.Phi0);
      phi1 = 1.45 / this.Phi0 * Beamlet + x;
      G1 = this.GainMax - 12 * ((phi_over_phi0 - x) ./ (Beamlet / this.Phi0)) .^ 2;
      
      G2 = this.GainMax - 25.23 + 40 * log10(phi1) - 40 * log10(phi_over_phi0);
      
      G = G0 .* (0    <= phi_over_phi0 & phi_over_phi0 <=  0.5) ...
          + G1 .* (0.5  <  phi_over_phi0 & phi_over_phi0 <= phi1) ...
          + G2 .* (phi1 <  phi_over_phi0);
