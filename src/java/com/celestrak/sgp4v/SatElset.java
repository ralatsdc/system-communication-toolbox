...
public class SatElset implements Serializable {
	...
	/**
	 * FIX: added US locale-specific format (decimal symbol causing problems at
	 * different locales)
	 */
	 ...
	 * 
	 * @param card1
	 *            the String containing line 1 of the elset
	 * @param card2
	 *            the String containing line 2 of the elset
	 */
	 ...
	 * 
	 * @param name
	 *            the String containing the satellite name
	 * @param card1
	 *            the String containing line 1 of the elset (plus possible
	 *            avgRCS value)
	 * @param card2
	 *            the String containing line 2 of the elset (plus possible TOES
	 *            value)
	 */
	public SatElset(String name, String card1, String card2) throws SatElsetException {
		...
		try {
			isValid = card1IsValid(line1) && card2IsValid(line2) && line1.regionMatches(2, line2, 2, 5);
	...
	public boolean card1IsValid(String card) throws SatElsetException, ValueOutOfRangeException {
		...
		if (card.length() < 68) {
			String errString = "SatElset.card1IsValid Card 1 length < 68 length = " + card.length();
			throw new SatElsetException(errString);
		}
		...
		try {
			satID = Integer.parseInt(card.substring(1, 7).replace(' ', '0'));
			if (satID < 1 || satID > 999999) {
				String errString = "SatElset.card1IsValid Card 1 satID number out of range *" + card.substring(2, 7)
						+ "*";
				throw new ValueOutOfRangeException(errString);
			}
		} catch (NumberFormatException ex) {
			String errString = "SatElset.card1IsValid Card 1 satID number format exception *" + card.substring(2, 7)
					+ "*";
		...
		if (!"U".equals(satClass) && !"C".equals(satClass) && !"S".equals(satClass) && !"T".equals(satClass)) {
			String errString = "SatElset.card1IsValid Card 1 classification error: [" + card.charAt(7) + "]";
			throw new SatElsetException(errString);
		}
		...
		try {
			...
			if (epochYr < 0 || epochYr > 99) {
				String errString = "SatElset.card1IsValid Card 1 epochYr out of range: [" + card.substring(18, 20)
						+ "]";
				throw new ValueOutOfRangeException(errString);
			}
		...
		try {
			...
			if (epochDay < 0.0 || epochDay > 367.0) {
				String errString = "SatElset.card1IsValid Card 1 epochDay out of range: [" + card.substring(20, 32)
						+ "]";
				throw new ValueOutOfRangeException(errString);
			}
		} catch (NumberFormatException ex) {
		...
		/*
		 * Now that we have the epochYr and epochDay, create a UTCTime with the
		 * epoch time of this sat, and store it as setUTCTime. (JRo steals code
		 * from Dominic B.)
		 */
		...
		try {
			...
			String errString = "SatElset.card1IsValid Invalid Ndot for Satellite number: " + satID + " ["
		...
		try {
			String tmpStr;
			if (card.charAt(50) == ' ') {
				tmpStr = card.substring(44, 45) + "." + card.substring(45, 50) + "E+" + card.substring(51, 52);
			} else {
				tmpStr = card.substring(44, 45) + "." + card.substring(45, 50) + "E" + card.substring(50, 52);
			}
			nDotDot = Double.valueOf(tmpStr).doubleValue();
		} catch (NumberFormatException exc) {
			String errString = "SatElset.card1IsValid Invalid NdotDot for Satellite number: " + satID + " ["
		...
		try {
			...
			bstar = Double.valueOf(card.substring(53, 54) + "." + card.substring(54, 59) + "E" + expString)
					.doubleValue();
		} catch (NumberFormatException exc) {
			String errString = "SatElset.card1IsValid Invalid BSTAR for Satellite number: " + satID + " ["
		...
		try {
			if (" ".equals(card.substring(62, 63))) {
				String errString = "SatElset.card1IsValid No Ephemeris Type for Satellite number: " + satID + " ["
						+ card.substring(62, 63) + "], Set to 0";
				throw new SatElsetException(errString);
			} else {
			...
			if (ephemerisType != 0 && ephemerisType != 2) {
				String errString = "SatElset.card1IsValid Card 1 ephemerisType out of range: [" + card.substring(62, 63)
		...
		if (card.length() > 68 && !" ".equals(card.substring(68, 69))) {
			if (checkSum(card) != Integer.parseInt(card.substring(68, 69))) {
				String errorString = "SatElset.card1IsValid Check Sum error on Card 1. Expecting: " + checkSum(card)
						+ " Actual value on card: " + Integer.parseInt(card.substring(68, 69)) + " Satellite number: "
						+ satID;
	...
	public boolean card2IsValid(String card) throws SatElsetException, ValueOutOfRangeException {
		...
		try {
			satID = Integer.parseInt(card.substring(1, 7).replace(' ', '0'));
			if (satID < 1 || satID > 999999) {
				String errorString = "SatElset.card2IsValid Card 2 satID number out of range: [" + card.substring(2, 7)
						+ "]";
				throw new ValueOutOfRangeException(errorString);
			}
		} catch (NumberFormatException ex) {
			String errString = "SatElset.card2IsValid Card 2 satID number format exception: [" + card.substring(2, 7)
					+ "]";
		...
		try {
			...
			if (testInclination < 0.0 || testInclination > 180.0) {
				String errorString = "SatElset.card2IsValid Card 2 inclination out of range: [" + card.substring(8, 17)
						+ "]  Satellite number: " + satID;
				throw new ValueOutOfRangeException(errorString);
			}
		...
		try {
			...
			if (testArgPerigee < 0.0 || testArgPerigee > 360.0) {
				String errorString = "SatElset.card2IsValid Card 2 argPerigee out of range: [" + card.substring(34, 42)
						+ "]  Satellite number: " + satID;
				throw new ValueOutOfRangeException(errorString);
			}
		...
		try {
			...
			if (testEccentricity < 0.0 || testEccentricity >= 1.0) {
				String errorString = "SatElset.card2IsValid Card 2 eccentricity out of range: [" + "0."
						+ card.substring(26, 33) + "]  Satellite number: " + satID;
				throw new ValueOutOfRangeException(errorString);
			}
			eccentricity = testEccentricity;
		} catch (NumberFormatException ex) {
			String errString = "SatElset.card2IsValid Card 2 eccentricity number format exception: [" + "0."
					+ card.substring(26, 33) + "]  Satellite number: " + satID;
			String errString2 = "SatElset.card2IsValid [" + ex + "]";
			throw new SatElsetException(errString + "\\n" + errString2);
		}
		...
		try {
			...
			if (testMeanAnomaly < 0.0 || testMeanAnomaly > 360.0) {
				String errorString = "SatElset.card2IsValid Card 2 meanAnomaly out of range: [" + card.substring(43, 51)
						+ "]  Satellite number: " + satID;
				throw new ValueOutOfRangeException(errorString);
			}
		...
		try {
			...
			if (meanMotion > 17.0) {
				String errorString = "SatElset.card2IsValid Card 2 meanMotion out of range: [" + card.substring(52, 63)
						+ "]  Satellite number: " + satID;
				throw new ValueOutOfRangeException(errorString);
			}
		} catch (NumberFormatException ex) {
		...
		try {
			...
			String errString = "SatElset.card2IsValid Card 2 revNum number format exception: [" + card.substring(63, 68)
					+ "]  Satellite number: " + satID;
			String errString2 = "SatElset.card2IsValid [" + ex + "]";
			throw new SatElsetException(errString + "\\n" + errString2);
		}
		...
		if (revNum < 0 || revNum > 99999) {
			String errorString = "SatElset.card2IsValid Card 2 revNum out of range: [" + card.substring(52, 63)
					+ "]  Satellite number: " + satID;
			throw new ValueOutOfRangeException(errorString);
		}
		...
		if (card.length() > 68 && !" ".equals(card.substring(68, 69))) {
			if (checkSum(card) != Integer.parseInt(card.substring(68, 69))) {
				String errString = "SatElset.card2IsValid Check Sum error on Card 2. Expecting: " + checkSum(card)
						+ " Actual value on card: " + Integer.parseInt(card.substring(68, 69)) + " Satellite number: "
						+ satID;
				throw new SatElsetException(errString);
			}
	 ...
	 * @param parts
	 *            the int array containing the sign, mantissa, and exponent
	 */
	...
	public String getCard1() {
		...
		if (exp.charAt(0) == '-') {
			...
			/**
			 * FIX: "0" in the exponent has a zero sign to get the checksums
			 * correct
			 */
			if (exp.charAt(0) == '0')
				card1sb.replace(44, 52, signStr + mantissa + "-" + exp);
		...
		card1sb.replace(68, 69, Integer.toString(checkSum(card1sb.substring(0, 68))));
	...
	public String getCard2() {
		...
		card2sb.replace(68, 69, Integer.toString(checkSum(card2sb.substring(0, 68))));
	 ...
	 * 
	 * @param card1
	 *            the String containing the replacement for line 1 of the elset
	 * @param card2
	 *            the String containing the replacement for line 2 of the elset
	 */
	public void set(String card1, String card2) throws SatElsetException, ValueOutOfRangeException {
		int slen = card1.length();
		String line1;
		String line2;
		if (slen <= 69) {
	 ...
	 * 
	 * @param name
	 *            the String containing the replacement for the satellite name
	 * @param card1
	 *            the String containing the replacement for line 1 of the elset
	 * @param card2
	 *            the String containing the replacement for line 2 of the elset
	 */
	public void set(String name, String card1, String card2) throws SatElsetException, ValueOutOfRangeException {
		if (name == null) {
			this.name = " ";
		} else {
			this.name = new String(name);
	 ...
	 * 
	 * @param argPer
	 *            the double containing the argument of perigee to use
	 *            [0.0,TWOPI] (rad)
	...
	public boolean setArgPerigee(double argPer) throws ValueOutOfRangeException {
		...
		String errString = "SatElset.setArgPerigee value: [" + argPer + "] out of range [0-2Pi]";
		throw new ValueOutOfRangeException(errString);
	}
	 ...
	 * 
	 * @param Bstar
	 *            the double containing the Bstar value to use
	 */
	 ...
	 *            the String containing the replacement for line 1 of the elset
	 */
	public void setCard1(String card) throws SatElsetException, ValueOutOfRangeException {
	 ...
	 *            the String containing the replacement for line 2 of the elset
	 */
	public void setCard2(String card) throws SatElsetException, ValueOutOfRangeException {
		int slen = card.length();
		String line2;
		if (slen <= 69) {
	 ...
	 * @return boolean true if the value provided is valid
	 * @param ecc
	 *            the double containing the eccentricity to use [>0.0,0.9999999]
	 */
	public boolean setEccentricity(double ecc) throws ValueOutOfRangeException {
		...
		String errString = "SatElset.setEccentricity value: [" + ecc + "] out of range (0-1)";
		throw new ValueOutOfRangeException(errString);
	}
	 ...
	 * @param elno
	 *            the int containing the element to use [0,9999]
	 */
	public boolean setElsetNum(int elno) throws ValueOutOfRangeException {
		...
		String errString = "SatElset.setElsetNum value: [" + elno + "] out of range [0-10000)";
		throw new ValueOutOfRangeException(errString);
	}
	 ...
	 * @param ephType
	 *            the int containing the ephemeris type to use [0 or 2]
	 */
	public boolean setEphemerisType(int ephType) throws ValueOutOfRangeException {
		if (ephType == 0 || ephType == 2) {
			ephemerisType = ephType;
			return true;
		}
		String errString = "SatElset.setEphemerisType value: [" + ephType + "] invalid [0,2]";
		throw new ValueOutOfRangeException(errString);
	}
	 ...
	 *         valid
	 * @param epochDay
	 *            the double containing the epoch day to use [00.0,366.99999999]
	 */
	public boolean setEpochDay(double epochDay) throws ValueOutOfRangeException {
		...
		String errString = "SatElset.setEpochDay value: [" + epochDay + "] out of range [0-367)";
		throw new ValueOutOfRangeException(errString);
	}
	 ...
	 * @param epochYr
	 *            the int containing the epoch year to use [00,99]
	 */
	 ...
	 * @param inclin
	 *            the double containing the inclination to use in radians
	 */
	public boolean setInclination(double inclin) throws ValueOutOfRangeException {
		if (inclin >= 0.0 && inclin <= Math.PI) {
			inclination = inclin;
			return true;
		}
		String errString = "SatElset.setInclination value: [" + inclin + "] out of range [0-Pi]";
		throw new ValueOutOfRangeException(errString);
	}
	...
	public boolean setIntDes(String intDes) throws ValueOutOfRangeException {
		if (intDes == null) {
			String errString = "SatElset.setIntDes value: [" + intDes + "] invalid [null]";
			throw new ValueOutOfRangeException(errString);
		}
		intDesig = intDes;
		return true;
	 ...
	 * @param meanAnom
	 *            the double containing the mean anomaly to use radians
	 */
	public boolean setMeanAnom(double meanAnom) throws ValueOutOfRangeException {
		...
		String errString = "SatElset.setMeanAnom value: [" + meanAnom + "] out of range [0-2Pi]";
		throw new ValueOutOfRangeException(errString);
	}
	 ...
	 * @param meanMotion
	 *            the double containing the mean motion to use [0.0,17.0]
	 *            (rev/day)
	 */
	public boolean setMeanMotion(double meanMotion) throws ValueOutOfRangeException {
		if (meanMotion > 0.0 && meanMotion <= 17.0) {
			this.meanMotion = meanMotion;
			return true;
		}
		String errString = "SatElset.setMeanMotion value: [" + meanMotion + "] out of range [0-17]";
		throw new ValueOutOfRangeException(errString);
	}
	 ...
	 * 
	 * @param name
	 *            the String containing the replacement sat name
	 */
	 ...
	 *         valid
	 * @param nDot
	 *            the double containing the n-dot to use (|nDot| < 1.0
	 *            rev/day/day)
	 */
	public boolean setNdot(double nDot) throws ValueOutOfRangeException {
		...
		String errString = "SatElset.setNdot value: [" + nDot + "] out of range [<1.0]";
		throw new ValueOutOfRangeException(errString);
	}
	 ...
	 *         derivative is valid
	 * @param nDotdot
	 *            the double containing the n-dotdot value to use (|nDot| < 1.0
	 *            rev/day^3)
	 */
	 ...
	 * @param revnum
	 *            the int containing the rev number to use [0,99999]
	 */
	public boolean setRevNum(int revnum) throws ValueOutOfRangeException {
		...
		String errString = "SatElset.setRevNum value: [" + revNum + "] out of range [0-100000)";
		throw new ValueOutOfRangeException(errString);
	}

	/**
	 * This method sets the right ascension of the ascending node on card 2 to
	 * the value supplied. The supplied value must be valid ([0.0,2Pi] radians)
	 * or false is returned.
	 * 
	 * @return boolean true if the value provided for right ascension is valid
	 * @param rtAsc
	 *            the double containing the right ascension to use in radians
	 */
	public boolean setRtAsc(double rtAsc) throws ValueOutOfRangeException {
		...
		String errString = "SatElset.setRtAsc value: [" + rtAsc + "] out of range [0-2Pi]";
		throw new ValueOutOfRangeException(errString);
	}
	 ...
	 * @param satID
	 *            the int containing the sat ID to use
	 */
	public boolean setSatID(int satID) throws ValueOutOfRangeException {
		// FUTURE 6-digit satellite number
		if (satID > 0 && satID < 1000000) {
			this.satID = satID;
			return true;
		}
		String errString = "SatElset.setSatID value: [" + satID + "] out of range (0-1000000)";
		throw new ValueOutOfRangeException(errString);
	}
	 ...
	 * @param secClass
	 *            the String containing the new classification (U, C, S, or T)
	 */
	public boolean setSecClass(String secClass) throws ValueOutOfRangeException {
		if ("U".equals(secClass) || "C".equals(secClass) || "S".equals(secClass) || "T".equals(secClass)) {
			satClass = secClass;
			return true;
		}
		String errString = "SatElset.setSecClass value: [" + secClass + "] out of range [U,C,S,T])";
		throw new ValueOutOfRangeException(errString);
	}
}
