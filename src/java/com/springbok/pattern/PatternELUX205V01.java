...
/**
 * Describes the ITU antenna pattern ELUX205V01.
 */
public class PatternELUX205V01 implements EarthPattern, TransmitPattern, ReceivePattern {
	...
	/**
	 * Constructs a PatternELUX205V01 given a maximum antenna gain.
	 *
	 * @param GainMax    Maximum antenna gain [dB]
	 * @param Efficiency Antenna efficiency, fraction
	 */
	public PatternELUX205V01(double GainMax, double Efficiency) {
		...
		this.d_over_lambda = Math.sqrt(Math.pow(10, (this.GainMax / 10)) / (Efficiency * Math.pow(Math.PI, 2)));
	...
	/**
	 * Copies a PatternELUX205V01 given a maximum antenna gain.
	 */
	public PatternELUX205V01 copy() {
		return new PatternELUX205V01(this.GainMax, this.Efficiency);
	}
	...
	public Gain gain(double Phi, Map options) {
		...
		double G3 = 7.9;
		double G4 = 32 - 25 * Math.log10(phi);
		double G5 = -10;

		double G = G0 * (0 <= phi && phi < phi_m ? 1 : 0) + G1 * (phi_m <= phi && phi < phi_r ? 1 : 0)
				+ G2 * (phi_r <= phi && phi < 7 ? 1 : 0) + G3 * (7 <= phi && phi < 9 ? 1 : 0)
				+ G4 * (9 <= phi && phi < phi_b ? 1 : 0) + G5 * (phi_b <= phi && phi <= 180 ? 1 : 0);

		double Gx = G;

		/* Validate low level rules */
		if (7 < phi_r) {
			throw new IllegalStateException("7 is less than phi_r [-6008: STDC_ERR_ANG2_LT_ANG1]");
		}
		if (GainMax < G1) {
	...
	public boolean equals(Object obj) {
		...
		if (!(obj instanceof PatternELUX205V01)) {
			return false;
		}
		PatternELUX205V01 other = (PatternELUX205V01) obj;
