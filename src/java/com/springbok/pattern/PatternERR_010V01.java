 ...
 */
public class PatternERR_010V01 implements EarthPattern, TransmitPattern {
	 ...
	 *            Maximum antenna gain [dB]
	 */
	public PatternERR_010V01(double GainMax) {
	 ...
	 */
	public PatternERR_010V01 copy() {
		return new PatternERR_010V01(this.GainMax);
	}
	...
	public Gain gain(double Phi, Map options) {
		...
		double G3 = GainMax - 28.0 - 25.0 * Math.log10(phi);
		double G4 = GainMax - 67.0;

		double G = G0 * (0 <= phi && phi <= 0.1 ? 1 : 0) + G1 * (0.1 < phi && phi <= 0.32 ? 1 : 0)
				+ G2 * (0.32 < phi && phi <= 0.54 ? 1 : 0) + G3 * (0.54 < phi && phi <= 36.31 ? 1 : 0)
				+ G4 * (36.31 < phi && phi <= 180 ? 1 : 0);

		double Gx = Math.min(GainMax - 35, G);
	...
	public boolean equals(Object obj) {
		...
		if (!(obj instanceof PatternERR_010V01)) {
			return false;
		}
		PatternERR_010V01 other = (PatternERR_010V01) obj;
