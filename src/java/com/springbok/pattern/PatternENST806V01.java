 ...
 */
public class PatternENST806V01 implements EarthPattern, TransmitPattern, ReceivePattern {
     ...
     * @param CoefA      Sidelobe reference level [dB]
     */
    public PatternENST806V01(double GainMax, double Efficiency, double CoefA) {
        ...
        if (this.d_over_lambda > 100) {
            ...
            this.G1 = this.CoefA - 50 + 25 * Math.log10(this.d_over_lambda);
     ...
     */
    public PatternENST806V01 copy() {
        return new PatternENST806V01(this.GainMax, this.Efficiency, this.CoefA);
    }

    /**
     * Non-standard generic earth station antenna pattern similar to that in
     * Recommendation ITU-R S.465-5, where the side-lobe radiation is represented by
     * the expression CoefA - 25 log(phi).
     *
     * @param Phi    Angle for which a gain is calculated [degrees]
     * @param optios Optional parameters (entered as key/value pairs)
    ...
    public Gain gain(double Phi, Map options) {
        ...
        double G2 = Math.max(CoefA - 25 * Math.log10(phi), -10);
    ...
    public boolean equals(Object obj) {
        ...
        if (!(obj instanceof PatternENST806V01)) {
            return false;
        }
        PatternENST806V01 other = (PatternENST806V01) obj;
